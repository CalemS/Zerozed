...
uint256 hashGenesisBlock("0xb345b68b4c08441d6b7f9f58f9fa83b0bce5b670afa7274e3f5725ea1be8fb03");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zerozed: starting difficulty is 1 / 2^12
...
{
    // Zerozed: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zerozed: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zerozed: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe41fe4a1e7b8325dd2f92f06b83a355dd1955c568d064af01a7949fafed6ae45");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042a5f52074836d012aa196582148e3afc76837f7d1eb57496adc08e0b174736ab5070e5644236113d56a4897664150a5d142437c2ae4b296ec95fc93ebde6cca1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526485560;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084675632;

        if (fTestNet)
        {
            block.nTime    = 1526485534;
            block.nNonce   = 385394553;
        }

        // Genesis block creation code incase you wanna fork me from scratch like a real alt instead of doing a shitty airdrop fork (BCH excluded. Bitcoin Cash is Bitcoin. #BCHPLS)
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        // Genesis block code incase you wanna fork me from scratch instead of doing a shitty airdrop fork (BCH excluded. Bitcoin Cash is Bitcoin. #BCHPLS)
        ...
        assert(block.hashMerkleRoot == uint256("0xc607bd15e58ec1213445c6f5d9059b1a865a30a527114a01c4dc3cac33e7f2eb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb5, 0xda }; // Zerozed Peer Magic
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZerozedMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZerozedMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZerozedMiner(CWallet *pwallet)
{
    printf("ZerozedMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zerozed-miner");
    ...
    try { loop {
        ...
        printf("Running ZerozedMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZerozedMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZerozedMiner, pwallet));
}
